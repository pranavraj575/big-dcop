...
from algo_configs import ALGORITHMS, get_algo_args

def run_pydcop(problem_file, algo_str, timeout, cycles=50):
    """
    Runs pydcop solve.
    ...
    if '(' in algo_str:
        alg_name = algo_str.split('(')[0]
        # remove outer parens and split by comma
        params_content = algo_str[algo_str.index('(') + 1:-1]
        alg_params_list = params_content.split(',')
    else:
        alg_name = algo_str
        alg_params_list = []
    ...
    cmd.extend(get_algo_args(alg_name))
...
def main():
    ...
    parser.add_argument("--cycles", type=int, default=50, help="Max cycles for local search algorithms")
    ...
    for p_idx, problem_path in enumerate(problem_files):
        ...
        for algo in ALGORITHMS:
            ...
            for i in range(args.trials):
                # Run the solver
                metrics = run_pydcop(problem_path, algo, args.timeout, args.cycles)
                ...
                print(f"        Trial {i+1}: {metrics.get('status')}, Cost: {metrics.get('cost', 'N/A')}, Time: {metrics.get('time', 'N/A')})")
